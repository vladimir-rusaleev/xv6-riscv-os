...
int main(int argc, char *argv[]) {
	...
	if((pid = fork()) < 0) {
		...
		printf("Родитель закрывает мьютекс\n");
		if( close(mutex_fd) < 0)
		...
		if(close(mutex_fd) < 0)
			...
			printf("Дочерний процесс попытался закрыть мьютекс, заблокированный родителем, \
но у него не получилось, перед этим не должно быть строки 'Мьютекс успешно закрыт'\n");
		exit(0);
	}
